...
from midst_models.single_table_TabDDPM.wb_pipeline_modules import *
from midst_models.single_table_TabDDPM.wb_pipeline_utils import *
...
def save_table_info(tables, relation_order, models, save_dir):
    ...
    for parent, child in relation_order:
        ...
        table_info[child].update(filtered_result)
...
def clava_synthesizing(table_names, relation_order, save_dir, all_group_lengths_prob_dicts, models, configs, sample_scale=1):
    ...
    for parent, child in relation_order:
        ...
        if parent is None:
            child_generated = sample_from_diffusion(
                meta_info=table_info[child], 
                df_info=result['df_info'], 
                diffusion=result['diffusion'],
                label_encoders=result['label_encoders'],
                sample_size=int(sample_scale * table_info[child]['size']),
            ...
            generated_final_df = generated_final_df[table_info[child]['columns']]
            ...
            original_columns = []
            for col in table_info[child]['columns']:
    ...
    for parent, child in relation_order:
        if child not in final_tables:
            if len(table_info[child]['parents']) > 1:
                final_tables[child] = handle_multi_parent(
                    child, 
                    table_info[child]['parents'], 
                    synthetic_tables, 
    ...
    for key, val in final_tables.items():
        cleaned_tables[key] = val[table_info[key]['original_cols']]
