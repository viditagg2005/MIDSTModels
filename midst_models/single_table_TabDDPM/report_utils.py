...
def baseline_load_synthetic_data(path, tables):
    ...
    for table, val in tables.items():
        syn[table] = {}
        syn[table]["df"] = pd.read_csv(
            os.path.join(path, "final", f"{table}_synthetic.csv")
        )
...
def clava_load_synthetic_data(path, tables):
    ...
    for table, val in tables.items():
        syn[table] = {}
        syn[table]["df"] = pd.read_csv(
            os.path.join(path, table, "_final", f"{table}_synthetic.csv")
        )
        syn[table]["domain"] = val["domain"]
    return syn
...
def sdv_load_synthetic_data(path, tables):
    ...
    for table, val in tables.items():
        syn[table] = {}
        syn[table]["df"] = pd.read_csv(os.path.join(path, f"{table}.csv"))
        syn[table]["domain"] = val["domain"]
    return syn
