...
from midst_models.multi_table_ClavaDDPM.pipeline_utils import load_multi_table
from midst_models.multi_table_ClavaDDPM.report_utils import (
    baseline_load_synthetic_data,
    clava_load_synthetic_data,
    get_multi_metadata,
    sdv_load_synthetic_data,
)
...
def recursive_merge(dataframes, keys):
    ...
    for i in range(
        ...
        result_df = pd.merge(
            left=result_df, right=dataframes[i], on=keys[i], how="left"
        )
    return result_df
...
def gen_multi_report(real_data_path, syn_data_path, syn_data_type, syn_data=None):
    ...
    multi_report = multi_eval_quality(
        real_tables, syn_tables, multi_metadata, verbose=False
    )
    ...
    for _, row in one_hop.iterrows():
        one_hop_dict[
            (row["Parent Table"], row["Child Table"], row["Column 1"], row["Column 2"])
        ] = row["Score"]
    ...
    for _, row in zero_hop.iterrows():
        zero_hop_dict[
            (row["Table"], row["Table"], row["Column 1"], row["Column 2"])
        ] = row["Score"]
    ...
    pickle.dump(result, open(os.path.join(syn_data_path, "multi_quality.pkl"), "wb"))
    return result
...
if __name__ == "__main__":
    ...
    gen_multi_report(args.real_data_path, args.syn_data_path, args.syn_data_type)
