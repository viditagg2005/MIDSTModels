...
def sample_from_diffusion(
    ...
    uniq_vals_list = meta_info['uniq_vals_list']

    num_numerical_features = meta_info['num_numerical_features']
    ...
    print(d_in)
    empirical_class_dist = meta_info['empirical_class_dist']
    x_gen, y_gen = diffusion.sample_all(sample_size, sample_batch_size, empirical_class_dist.float(), ddim=False)
    X_gen, y_gen = x_gen.numpy(), y_gen.numpy()
    num_numerical_features_sample = num_numerical_features + int(meta_info['is_regression'] and not model_params["is_y_cond"])

    num_num_cols = len(df_info['num_cols'])
    num_cat_cols = len(df_info['cat_cols'])
    X_num_ = X_gen

    if num_numerical_features != 0:
        X_num_ = meta_info['inverse_transform'](X_gen[:, :num_numerical_features_sample])
        ...
        disc_cols = []
        for col in range(num_num_cols):
            uniq_vals = uniq_vals_list[col]
            if len(uniq_vals) <= 32 and ((uniq_vals - np.round(uniq_vals)) == 0).all():
                disc_cols.append(col)
        print("Discrete cols:", disc_cols)
        if len(disc_cols):
            X_num = round_columns_uniq_vals(uniq_vals_list, X_num, disc_cols)
    ...
    columns = [str(x) for x in range(df_gen.shape[1])]

    df_gen.columns = columns

    for col in df_gen.columns:
        if int(col) < num_num_cols:
            df_gen[col] = df_gen[col].astype(float)
        elif num_cat_cols > 0 and int(col) < num_num_cols + num_cat_cols:
...
def train_model(
    ...
    return {
        ...
        'inverse_transform': dataset.num_transform.inverse_transform
    }
...
def conditional_sampling_by_group_size(
    ...
    num_numerical_features_sample = num_numerical_features + int(meta_info['is_regression'] and not is_y_cond)
    ...
    return df_gen, sampled_group_sizes
