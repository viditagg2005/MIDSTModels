...
from midst_models.multi_table_ClavaDDPM.eval.eval_detection import eval_detection
from midst_models.multi_table_ClavaDDPM.eval.eval_mle import eval_mle
from midst_models.multi_table_ClavaDDPM.eval.eval_quality import eval_metrics
...
def gen_single_report(
    ...
    if len(domain_dict) > 1:
        if not os.path.exists(
            os.path.join(save_path, f"{table_name}_alpha.pkl")
        ) or not os.path.exists(os.path.join(save_path, f"{table_name}_beta.pkl")):
            alpha, beta = compute_alpha_beta(
                real_data, syn_data, domain_dict, sample_size=alpha_beta_sample_size
            )
            ...
            alpha = pickle.load(
                open(os.path.join(save_path, f"{table_name}_alpha.pkl"), "rb")
            )
            beta = pickle.load(
                open(os.path.join(save_path, f"{table_name}_beta.pkl"), "rb")
            )
    ...
    if len(domain_dict) > 1:
        if test_data is not None and len(domain_dict) > 1 and len(test_data) > 0:
            if not os.path.exists(os.path.join(save_path, f"{table_name}_mles.pkl")):
                ...
                mles = pickle.load(
                    open(os.path.join(save_path, f"{table_name}_mles.pkl"), "rb")
                )
                print(get_avg(mles))
                print()
    ...
    if not os.path.exists(os.path.join(save_path, f"{table_name}_detection.pkl")):
        ...
        detection_score = pickle.load(
            open(os.path.join(save_path, f"{table_name}_detection.pkl"), "rb")
        )
